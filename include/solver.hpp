...
private:
    ...
    R _gamma;
    int _saved_count;
    DistributedGrid<R, NDims> _grid_curr;
    DistributedGrid<R, NDims> _grid_next;

    auto save() -> void {
        MPI_File fh;
        auto filename{std::format("state_{}.ppm", _saved_count++)};
        MPI_File_open(_grid_curr.comm(), filename.c_str(),
        ...
        auto &curr{_grid_curr.local_grid()};
        ...
        std::array<int, 2> image_local_size{_grid_curr.local_size()[1],
                                            _grid_curr.local_size()[0] * 3};
        std::array<int, 2> image_local_start{_grid_curr.local_start()[1],
                                             _grid_curr.local_start()[0] * 3};
...
public:
    ...
    auto run() -> void {
        ...
        while (!converged) {
            ...
            _grid_curr.synchronize_halos(_dirichlet_boundary_conditions);

            // Solve
            auto total_norm_delta{R{0}};
            auto &curr{_grid_curr.local_grid()};
            auto &next{_grid_next.local_grid()};
            auto inner_indices{_grid_curr.inner_coordinates()};
